...
2. **索引がページ単位で章に割れる** → `split_chapters_and_nav()` は直前の大見出しと
   ...
   重複見出し 10 ブロック＝参考文献 6・事項索引 3・人名索引 1 のみ、非見出しブロックの
   欠落 0）。既知の限界: タイトルが同じで中身が別の節（複数人の「解説」など）は
   2 件目の見出しが消えて連結される。内容は残るが節の切れ目は見えなくなる。
3. **前書き中の部構成の説明文が部見出しとして立ち、本文を吸い込む** →
   ...
   **正規化前の生の OCR テキスト**で計測した実測値は 本物の扉 0 字 / 偽の見出し
   650・617 字 / 表紙を除く最薄のページ 118 字）。本物の扉がその本に 1 つも無ければ
   何もしない。
   比較する範囲は部番号の信頼度で切り替える。扉ごとに番号が区別できていれば同じ部番号
   どうしで、番号が潰れていれば本全体で比較する。番号は OCR のいちばん外しやすい部分で
   （PDF テキスト層が無いと `book_ir.fix_bu_numerals()` が働かず Ⅱ・Ⅲ が Ⅰ に化ける）、
   常に番号を鍵にすると『時間を哲学する』で降格できず、常に本全体で比較すると扉に
   献辞を置く体裁の部を降格してしまうため。
